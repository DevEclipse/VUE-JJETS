...
const methods = {
  methods: {
    ...
    addUser(user) {
      this.addProfile(user,'manager');
      this.addProfile(user,'employee');
      this.addProfile(user,'customer');
      this.$firebaseRefs.users.child(user.username.toLowerCase()).set(user);
    },
    addProfile(user,profileType) {
      switch(profileType) {
        case 'manager':
    ...
    //Delete Methods
    deleteUser(user) {
      this.$firebaseRefs.users.child(user.username).remove();
      this.$firebaseRefs.managers.child(user.username).remove();
      this.$firebaseRefs.employees.child(user.username).remove();
      this.$firebaseRefs.customers.child(user.username).remove();
    },
    deleteStore(store) {
